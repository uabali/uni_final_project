...
interface ChatState {
    ...
    setMessageMeta: (conversationId: string, messageId: string, meta: { latency_ms?: number; token_count?: number }) => void;
...
function hydrateDates(state: ChatState): ChatState {
    return {
        ...state,
...
export const useChatStore = create<ChatState>()(
    persist(
        ...
        {
            name: "frappe-chat-storage",
            merge: (persisted, current) => {
                if (!persisted) return current;
                const hydrated = hydrateDates({
                    ...current,
                    ...persisted,
                } as ChatState);
                return hydrated;
